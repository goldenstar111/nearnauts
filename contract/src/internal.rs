...
impl Contract {
	...
	pub(crate) fn assert_mint_yocto(&self) {
		assert!(
			env::attached_deposit() >= self.mint_price,
			"Requires attached deposit of exactly mint price yoctoNEAR",
		)
	}
	
	//refund the initial deposit based on the amount of storage that was used up
	pub(crate) fn refund_deposit(&self, storage_used: u64) {
